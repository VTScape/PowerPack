...
// Date: 12/15/2018
// Description: The parallelClient is not actually parallel, it is just a client
// for the parallelServer code. It simply accepts a hostname, then a port
// number, then a file to stream from and a file to stream to
//
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    ...
    fprintf(stderr,
            "Can you please just conform to the usage: %s hostname port "
            "fileToStreamFrom fileToStreamTo\n",
            argv[0]);
    exit(0);
  }
  ...
  if (sockfd < 0) error("ERROR opening socket");  // prints to sderr
  server = gethostbyname(argv[1]);                // stores the host name
  if (server == NULL) {
    fprintf(stderr, "ERROR, no such host\n");  // if there is a problem finding
                                               // the host name, make an error
    exit(0);
  }

  // finishes setting up the communications
  bzero((char *)&serv_addr,
        sizeof(serv_addr));  // zeroes the server address holder
  serv_addr.sin_family =
      AF_INET;  // tells the server address struct to use IPv4
  bcopy((char *)server->h_addr, (char *)&serv_addr.sin_addr.s_addr,
        server->h_length);  // copies the server's h_addr to the
                            // serv_addr.sin_addr.s_addr
  serv_addr.sin_port =
      htons(portno);  // converts the unsigned short integer hostshort from host
                      // byte order to network byte order.
  clock_gettime(CLOCK_REALTIME, &startTime);
  if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) <
      0)                        // attempts to connect sockfd to
  ...
  // prints the amount of time from sending the request to receiving the sorted
  // array
