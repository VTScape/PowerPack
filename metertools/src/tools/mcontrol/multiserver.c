...
#define	DEFAULT_METER_PORT	6813 //6998
#define RAPL_HOST "172.16.0.13"
...
unsigned int monitor_rapl(void *);
...
static connection_t *connection,*connection_api;
static float power;
...
static netmeter_paras net_wattsup = {"wattsup", "172.16.0.240", 6913, 1, 1000, 0, (timely_energy *)&timely_wattsup};
static netmeter_paras net_nidaq = {"nidaq", "172.16.0.100", 6913, 25, 20, 0, (timely_energy *)&timely_nidaq};
...
void *request_handler_fAPI(void *arg) {
   int s;
   struct sockaddr_in raddr;
   int rlen;
   int i;
...
//   printf("------------------tag\n");
   ...
   while (running) {
      ...
      for (i = 0; ; i++) {
         ...
         if (connection_api[i].sock == 0)
            break;
      }
      poll(&pfd, 1, 1000);
      if (pfd.revents == POLLIN) {
         connection_api[i].sock = accept(s, (struct sockaddr *)&raddr, &rlen);
         if (connection_api[i].sock < 0) {
            connection_api[i].sock = 0;
            fprintf(stderr, "connection_api %d: error accept: %s\n", i,
               strerror(errno));
            continue;
         } else {
	    printf("request_handler: connection_api(%d) threading client_req_fAPI\n", i);

            if (pthread_create(&connection_api[i].thread, &thr_attr,
//                  client_req_fAPI, (void * (*)(void *))i) != 0) {
                  client_req_fAPI, (void *)i) != 0) {
               fprintf(stderr, "connection_api %d: error pthread_create: %s\n",
                  i, strerror(errno));
               close(connection_api[i].sock);
               connection_api[i].sock = 0;
            }
         }
      }
...
void *client_req_fAPI(void *arg) {
   ...
   gettimeofday(&start, NULL);
   connection_api[id].id = pthread_self();
	printf("------------- tag %u\n",pthread_self());
//   pthread_detach(connection_api[id].id);
//	printf("------------- tag %u\n",pthread_self());
   if (getpeername(connection_api[id].sock, (struct sockaddr *)&fsin,
     &alen) == -1)
      return (void *)-1; /* client abort */
   printf("[API]connection_api %d: start client %s\n", id, inet_ntoa(fsin.sin_addr));
   while (running && connected) {
      memset(msgbuf, 0, sizeof(msgbuf));
      memset(&msg, 0, sizeof(msg));

      
      if ((size = read(connection_api[id].sock, msgbuf, sizeof(msgbuf))) == -1 ){
         fprintf(stderr, "[API]connection_api %d: error request size %d errno %d\n",
            id, size, errno);
         connected = 0;
      } else if (!running) {
         connected = 0;
	 ...
	 if ( size == sizeof( netmeter_req_t )){
         ...
         switch(msg.cmd) {
            case NETMETER_CMD_ENERGY:
               send_energy(connection_api[id].sock); commands++;
               break;
            case NETMETER_CMD_POWER:
               send_power(connection_api[id].sock); commands++;
               break;
            case NETMETER_CMD_READ:
               send_reading(connection_api[id].sock); commands++;
               break;
            case NETMETER_CMD_RESET:
               cumulative_energy = 0;
               commands++;
            ...
            case NETMETER_CMD_QUIT: /* gracefully exit */
               ...
               break;
            default:
               fprintf(stderr, "[API]connection_api %d: error unknown command %i\n",
                  id, msg.cmd);
               connected = 0;
               break;
		 }
...
//						printf("----------->>> END_LOG_FILE ---->>>\n");
 ...
 //  pthread_detach(connection_api[id].id);
   close(connection_api[id].sock);
   connection_api[id].sock = 0;
   gettimeofday(&stop, NULL);
   sec = stop.tv_sec - start.tv_sec;
   if (sec >= 3600) { /* seconds per hr */
      hr = sec / 3600;
   ...
   printf("connection_api %d: stop service time %uh:%um:%.3fs for %lu command%s\n",
      id, hr, min, sec + (stop.tv_usec / 1000000.0), commands,
      commands == 1 ? "": "s");
...
void initialize_API(pthread_t *thread, unsigned short port) {
   signal(SIGINT, signal_exit);
   if (port == 0)
      return;
   connection_api = (connection_t *)calloc(MAXCONNS, sizeof(connection_t));
   if (connection == NULL) {
      fprintf(stderr, "out of memory creating %d connections\n", MAXCONNS);
      exit(1);
   }
   if (pthread_create(thread, NULL, request_handler_fAPI, (void *)(int)6713) != 0) {
      fprintf(stderr, "unable to start network listener: %s\n",
         strerror(errno));
      exit(1);
   }
...
int main(int argc, char *argv[]) {
   ...
   pthread_t thread, netmeter_thread, threadAPI, threadRapl;
   ...
   if (pthread_create(&threadRapl, NULL, monitor_rapl, (void *)1) != 0) {
      fprintf(stderr, "unable to start rapl connection: %s\n",
         strerror(errno));
      exit(1);
   }
   initialize(&thread, port);
   initialize_API(&threadAPI, 6713);
/*
   // ------ Modification by Bo Li ------
//   initialize(&threadAPI,6997);
   if (pthread_create(&threadAPI, NULL, request_handler_fAPI, (void *)6997) != 0) {
      fprintf(stderr, "unable to start network listener: %s\n",
         strerror(errno));
      exit(1);
   }
...
/**
 * Collect Rapl data from n13. (Bo Li) 04/11/2013
 */
unsigned int ch_pwr_readings_rapl[4];

unsigned int monitor_rapl(void *vp){
   ...
   if ((sock = netmeter_connect(RAPL_HOST, 6999)) == -1) {
   	  fprintf(stderr, "rapl_connect: %s\n", strerror(errno));
      running = 0;
      return -1;
   }
   msg.cmd = cmd;
   ...
   while (running) {
     r = netmeter_request(sock, &msg, ch_pwr_readings_rapl);
     if (r < 0) {
        fprintf(stderr, "netmeter_request: request to rapl server failed %d\n", r);
        continue;
     }
     usleep(50000);
     //printf("%u\n",ch_pwr_readings_rapl[0]);
   }
  return 0;
}
...
unsigned int monitor_netmeter(void *arg) {
   ...
   while (running && loops >= 0) {
      ...
      // -------Modification by Bo Li--------
      // fplog timely_nidaq and timely_wattsup into file.
      if(fplog!=0){
//        fprintf(fplog, "%.2f\t \t", timely_netmeter->sys_power/1000.0);
        fprintf(fplog, "%.2f\t", now.tv_sec + now.tv_usec * 1.0e-6);
        if(msg.num_chs>1){
	  fprintf(fplog, "%.2f\t \t", timely_wattsup.sys_power/1000.0);
	}
	else{
          fprintf(fplog, "%.2f\t \t", timely_netmeter->sys_power/1000.0);
	}
        double cpuP=0.0,memP=0.0;
	fprintf(fplog, "%.2f\t", (timely_netmeter->nidaq_powers[0]+timely_netmeter->nidaq_powers[1]+timely_netmeter->nidaq_powers[2]+
          timely_netmeter->nidaq_powers[3]+timely_netmeter->nidaq_powers[4]+timely_netmeter->nidaq_powers[5]+
          timely_netmeter->nidaq_powers[6]+timely_netmeter->nidaq_powers[7]+timely_netmeter->nidaq_powers[8]+
          timely_netmeter->nidaq_powers[9]+timely_netmeter->nidaq_powers[10]+timely_netmeter->nidaq_powers[11]+
          timely_netmeter->nidaq_powers[12]+timely_netmeter->nidaq_powers[13]+timely_netmeter->nidaq_powers[14]+
	  timely_netmeter->nidaq_powers[15])/1000.0);
   pthread_mutex_lock(&mutex);
        cpuP=(ch_pwr_readings_rapl[0])/1000.0;
	fprintf(fplog, "%.2f\t", cpuP);
	if((ch_pwr_readings_rapl[2]/1000.0)>0.63){
	  memP=11.16709-ch_pwr_readings_rapl[2]*4.2069/1000.0;
	  fprintf(fplog, "%.2f\t", memP);
	}
	else{
	  memP=13.0378-ch_pwr_readings_rapl[2]*5.20957/1000.0;
	  fprintf(fplog, "%.2f\t", memP);
	}
	fprintf(fplog, "||\t%.2f\t||\t",ch_pwr_readings_rapl[3]/1000.0);
   pthread_mutex_unlock(&mutex);
	fprintf(fplog, "%.2f\t||\t", (timely_netmeter->nidaq_powers[0]+timely_netmeter->nidaq_powers[1]+timely_netmeter->nidaq_powers[2]+
          timely_netmeter->nidaq_powers[3]+timely_netmeter->nidaq_powers[4]+timely_netmeter->nidaq_powers[5]+
          timely_netmeter->nidaq_powers[6]+timely_netmeter->nidaq_powers[7]+timely_netmeter->nidaq_powers[8]+
	  timely_netmeter->nidaq_powers[8])/1000.0);

        int k=0;
        for(k=0;k<timely_nidaq.num_nidaq_chs;k++){
          fprintf(fplog, "%d\t", timely_netmeter->nidaq_powers[k]);
        }
        fprintf(fplog, "\n");
