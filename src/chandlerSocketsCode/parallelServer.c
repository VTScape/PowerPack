...
// Date: 12/16/2018
// Description: The parallelServer takes connections from clients, first
// receiving an integer telling how long the list of integers to sort will be,
// then receiving the list of integers to be sorted, then receiving an integer
// telling it how many sorts to perform before exiting
//
...
void *handleClient(void *socketNumber) {
  ...
  n = read(newsockfd, &listSize,
           sizeof(int));  // read the number of integers in the list
  if (n < 0)
    error("ERROR reading buffer length from socket");  // throw an error
  int *buffer = malloc(sizeof(int) *
                       listSize);  // allocate memory for the list to be sorted
  n = read(
      newsockfd, buffer,
      sizeof(int) * listSize);  // read in the list to be sorted into the buffer
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    fprintf(stderr,
            "ERROR, provide a port and a number of connections to accept\n");
    exit(1);
  }

  sockfd = socket(AF_INET, SOCK_STREAM, 0);  // attempts to open a socket
  if (sockfd < 0)
    error("ERROR opening socket");  // throws an error if the socket is unable
                                    // to be opened

  bzero((char *)&serv_addr,
        sizeof(serv_addr));  // zeroes all the server address data
  portno = atoi(argv[1]);  // converts the argument with the port number from a
  ...
  for (connectionCounter = 0; connectionCounter < maxConnections;
       connectionCounter++) {
    ...
    newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr,
                       &clilen);  // accept the connection
    if (newsockfd < 0)
      error("ERROR on accept");  // throw an error if the accept failed
    pthread_create(&pThreadHolder[connectionCounter], NULL, handleClient,
                   (void *)newsockfd);
  }
