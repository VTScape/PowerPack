...
void streamFromFile(char *fileName, int sockfd) {
  ...
  */
  ...
  while (readCounter == FILE_INTEGERS) {
    ...
    n = write(sockfd, &readCounter,
              sizeof(int));  // writes how many integers to expect to the socket
    if (n < 0) {
    ...
    n = write(
        sockfd, buffer,
        sizeof(int) *
            readCounter);  // writes the integers to be sorted to the stream
    if (n < 0) {
...
void streamToFile(char *fileName, int sockfd) {
  ...
  while (readCounter == FILE_INTEGERS) {
    ...
    // readCounter == FILE_INTEGERS*sizeof(int)/sizeof(char)); printf("Reading
    // the size of the file to accept\n");
    n = read(sockfd, &readCounter,
             sizeof(int));  // read how many characters to receive
    ...
    if (readCounter > 0) {
      ...
      if (readCounter > FILE_INTEGERS) {
        printf(
            "The readCount was too high, stopping the program before a "
            "segfault, readcounter was %d\n",
            readCounter);
      ...
      n = read(
          sockfd, buffer,
          sizeof(int) *
              readCounter);  // reads the integers to be written to the file
      if (n < 0) {
